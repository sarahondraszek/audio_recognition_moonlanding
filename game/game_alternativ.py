...
def flag(status, rocket, moon, pathflag_in, pathflag_out):
    class Flag(pygame.sprite.Sprite):
        def __init__(self):
            super(Flag, self).__init__()
            self.flag_in = resource_path(pathflag_in)
...
def game():
    ...
    while True:
        ...
        if quit:
            pygame.quit()
            break
        quit, status, rocket, moon = main(path, pathfire, difficulty, pathflag_in, pathflag_out)
        if quit:
            pygame.quit()
            break
        flag(status, rocket, moon, pathflag_in, pathflag_out)
